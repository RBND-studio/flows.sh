...
- A **tour that never started** because another tour was already running: with **Show multiple tours at the same time** off (the default), only the tour with the most specific trigger starts and the others simply wait for their trigger to match again. See [tours.md](tours.md#running-multiple-tours).

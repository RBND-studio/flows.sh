...
| `signature`        | `string \| null`    | No       | HMAC signature of `userId`, computed on the backend. Required once identity verification is enforced, see [identity-verification.md](identity-verification.md).If set to `null`, the SDK will be disabled, this is useful when loading the signature asynchronously. |

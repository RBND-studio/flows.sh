...
Custom steps are UI components from your component library that you can add into a tour. To make a component available as a tour step, create it with the **Tour component** type. See [component types](/components/custom#component-types).
...
To ensure compatibility, call the matching `continue`, `previous` and `cancel` props that `TourComponentProps` gives you.
...
Under **Settings**, the **When user session ends** property decides what happens to a half-finished tour when the user leaves your app and does not come back for a few minutes. You can preserve their progress, restart the tour, or end it through its `complete` or `cancel` exit node. The same setting decides how the tour is resolved if another tour interrupts it.

See [tour sessions](/tours/sessions) for the available actions and how to choose between them.

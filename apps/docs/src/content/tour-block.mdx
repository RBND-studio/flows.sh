...
Tours consist of tour-specific blocks from your block library. Each tour step functions like a regular block but comes with built-in connections to the previous and next steps via the `previous` and `continue` exit nodes. Additionally, a `cancel` exit node lets users exit the tour at any point.

Tours also feature an integrated wait step that pauses the tour until the user interacts with your application—such as clicking a button or navigating to a new page.
...
- Use built-in wait steps that pause the tour until the user interacts with your application (for example, by clicking a button or navigating to a specific page).

![Tour steps](/workflows/tour-steps.png)

### Custom steps

Custom steps are blocks from your block library that you can add into a tour. When creating a [block template](/block-templates), you can designate a block as a tour step, making it available for tours.

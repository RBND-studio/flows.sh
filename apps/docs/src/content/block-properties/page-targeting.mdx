...
Page targeting allows you to determine where a block appears based on the current page [pathname](https://developer.mozilla.org/en-US/docs/Web/API/URL/pathname). This is useful for displaying blocks on specific pages or sections of your application. In [wait step](/tour-block#wait-step) and [wait properties](/block-properties/wait), page targeting can be used to pause the workflow until the user navigates to a specific page or does a specific action on the desired page.

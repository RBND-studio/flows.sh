...
## Defining a State memory property

When you set up a State memory property, you can specify common attributes like title, description, and key. The boolean value is automatically stored in the block's state and can be accessed by using the property key in your application.

## Configuring a State memory property in a workflow

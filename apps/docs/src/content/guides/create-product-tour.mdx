...
We recommend publishing to a **staging environment** if you can (see [environments docs](/environments) to learn how to add a new environment). If you’re testing in production, limit access using user property filters in the **Start** block (eg. `admin = true` or `email = your@email.com`), learn more about how to [filter users using the Start block](/start-blocks/automatic-start).

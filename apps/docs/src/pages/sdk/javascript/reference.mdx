...
Helps with defining a custom component for use with `@flows/js-components`. Define the props that you have entered in your [block template](/blocks/templates) and return object with root `element` and `cleanup` function that will be run before the element is removed from the DOM.

```ts filename="modal.ts"

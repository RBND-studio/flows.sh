...
User properties can be used to segment users in [workflows](/workflows) by creating conditions in [blocks](/blocks/library) to filter users based on their properties. These blocks include:

- **[Start block](/blocks/library#start-blocks)** - filter which users can enter a workflow
- **[Filter block](/blocks/library#filter)** - allow only users with specific properties to proceed

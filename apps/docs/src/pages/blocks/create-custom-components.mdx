...
```tsx {1, 3} filename="tour-banner.tsx"
...
If your component is intended to be rendered inside the application, you need to select the **Slottable** option. This will allow you to specify a slot id when adding the block to a workflow. In our case, the banner component is [slottable](/workflows/configure-blocks#slot-configuration) because we want to embed it into our application UI.

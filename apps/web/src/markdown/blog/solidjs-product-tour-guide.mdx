---
title: How to build a product tour in SolidJS
description: A complete guide to building a product tour in SolidJS. Learn how to create tooltips, modals, onboarding flows, track user progress, and ship tours faster using Flows.
date: "2026-01-20"
author: "Vojtech Vidra"
image: /images/blog/product-tour-guide/solidjs-cover.png
imageAlt: Tooltip example for a product tour in a SolidJS application
---

import { VideoPlayer } from "components/blog";

In this guide, you'll learn how to build a product tour (also known as a user onboarding flow or product adoption guide) in your SolidJS application. Whether you're using SolidStart or a plain Vite + Solid setup, this tutorial will walk you through everything you need to know.
...
In SolidJS, general-purpose tour libraries like `shepherd.js`, `intro.js`, and `driver.js` can be wired up manually.
...
You might be tempted to store user state in the browser's local storage, but for any SolidJS app where users sign in (especially if they may use multiple devices) the state should be stored in your database instead.
...
## Build your SolidJS product tour faster with Flows
...
Flows initializes client-side and works alongside SolidJS's fine-grained reactivity. It manages its own internal state independently from your signal graph, so adding Flows won't introduce reactive primitives you didn't ask for. For UI, you can bring your own SolidJS components directly from your design system. Flows controls when they appear, you control how they look.
...
After adding all your steps, the tour is ready for a preview. To preview the tour, create a testing environment called `stage` by going to **Settings > Environments**. Then change the `environment` option passed to `init()` to match the newly created environment.
...
Now, when you start your SolidJS app, you should see the first step of the tour (you may need to reload the page if the app was already running before you changed the environment). If you have any issues, you can debug by looking into the browser console or by using our SDK's [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your product tour looks and feels, you can publish it to your users. First deploy the updated SolidJS app with the `init()` integration (don't forget to change the environment option back to `production`).
...
<VideoPlayer
  ...
  ariaLabel="Preview of the product tour in a SolidJS application"
/>

---

That's it, you've successfully built a complete tour in SolidJS using Flows.
...
- [How to build a product tour in JavaScript](/blog/javascript-product-tour-guide)

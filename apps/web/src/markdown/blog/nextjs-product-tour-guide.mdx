---
title: How to build a product tour in Next.js
description: A complete guide to building a product tour in Next.js. Learn how to create tooltips, modals, onboarding flows, track user progress, and ship tours faster using Flows.
date: "2026-01-06"
author: "Vojtech Vidra"
image: /images/blog/product-tour-guide/nextjs-cover.png
imageAlt: Tooltip example for a product tour in a Next.js application
---

import { VideoPlayer } from "components/blog";

In this guide, you'll learn how to build a product tour (also known as a user onboarding flow or product adoption guide) in your Next.js application. Whether you're using the App Router or Pages Router, this tutorial will walk you through everything you need to know.
...
In Next.js, you can use the same React-based tour packages as any React app: `react-joyride`, `shepherd.js`, `intro.js`, `driver.js`, or `reactour`.
...
You might be tempted to store user state in the browser's local storage, but for any Next.js app where users sign in (especially if they may use multiple devices) the state should be stored in your database instead.
...
## Build your Next.js product tour faster with Flows
...
Now, when you start your Next.js app, you should see the first step of the tour (it is possible that you will need to reload the page if you already had the app running before changing the environments). If you have any issues, you can debug by looking into the browser console or by using our SDK's [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your product tour looks and feels, you can publish it to your users. To do that, you first need to deploy the updated Next.js app with `<FlowsProvider>` (don't forget to change the environment prop back to `production`).
...
<VideoPlayer
  ...
  ariaLabel="Preview of the product tour in a Next.js application"
/>

---

That's it, you've successfully built a complete tour in Next.js using Flows.

---
title: How to build an onboarding checklist in SolidJS
description: A complete guide to building a user onboarding checklist in SolidJS. Learn how to create a floating checklist widget, track task completion, and ship activation flows faster with Flows.
date: "2026-07-06"
author: "Ondrej Pesicka"
image: /images/blog/onboarding-checklist-guide/solidjs-cover.png
imageAlt: Floating onboarding checklist example in a SolidJS application
---

In this guide, you'll learn how to build an onboarding checklist (also known as a floating checklist, setup guide, or getting started widget) in your SolidJS application. Whether you're using SolidStart or a plain Vite + Solid setup, this tutorial will walk you through everything you need to know.
...
<Image
  ...
  alt="An example of a floating onboarding checklist in a SolidJS application"
  width={1800}
  height={1040}
/>
...
The SolidJS ecosystem has no dedicated package for onboarding checklists, so most teams assemble one from primitives (e.g. Popover, Checkbox, and Progress from `@kobalte/core`) and write custom logic for opening, collapsing, and persisting the widget across pages.
...
Flows initializes client-side and works alongside SolidJS's fine-grained reactivity. It manages its own internal state independently from your signal graph, so adding Flows won't introduce reactive primitives you didn't ask for.
...
Create a `Flows` component that initializes the SDK inside `onMount` and renders the floating blocks, then add it to your root layout:
...
For the full list of supported options and a complete example, see the [SolidJS installation guide](/docs/sdk/frameworks/solid) or the [SolidJS template project](https://github.com/RBND-studio/flows-sdk/tree/main/examples/solid). When that is set up, come back here.
...
Now, when you start your SolidJS app, you should see the checklist widget in the corner of the screen (it is possible that you will need to reload the page if you already had the app running before changing the environments). If you have any issues, you can debug by looking into the browser console or by using our SDK's [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your checklist looks and feels, you can publish it to your users. To do that, you first need to deploy the updated SolidJS app with the `init()` integration (don't forget to change the environment option back to `production`).
...
That's it, you've successfully built a floating onboarding checklist in SolidJS using Flows.
...
If you're ready to ship better onboarding, faster, [try Flows](/technology/solid-user-onboarding).
...
And if you want to guide users through a linear sequence of steps instead, see [How to build a product tour in SolidJS](/blog/solidjs-product-tour-guide).

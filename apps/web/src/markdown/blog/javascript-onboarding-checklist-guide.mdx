---
title: How to build an onboarding checklist in JavaScript
description: A complete guide to building a user onboarding checklist in JavaScript. Learn how to create a floating checklist widget, track task completion, and ship activation flows faster with Flows. No framework required.
date: "2026-07-06"
author: "Ondrej Pesicka"
image: /images/blog/onboarding-checklist-guide/javascript-cover.png
imageAlt: Floating onboarding checklist example in a JavaScript application
---

In this guide, you'll learn how to build an onboarding checklist (also known as a floating checklist, setup guide, or getting started widget) in a JavaScript application. Whether you're using vanilla JS, a legacy codebase, or a lightweight stack without a major framework, this tutorial will walk you through everything you need to know.
...
<Image
  ...
  alt="An example of a floating onboarding checklist in a JavaScript application"
  width={1800}
  height={1040}
/>
...
There is no go-to JavaScript package for onboarding checklists, so in a plain JavaScript app you'd build the widget from scratch with DOM APIs or web components: a floating button, a popup, a task list, and a progress bar, plus the logic for opening, collapsing, and persisting the widget across pages.

#### Checklists in single-page vs multi-page apps

In a traditional multi-page app (MPA), each navigation is a full page reload, so the widget and its state need to be re-created on every page. In a single-page app (SPA) with a client-side router, the widget initializes once and needs to survive route changes. A checklist has to work reliably in both cases, since it follows the user around your app for days or weeks.
...
The Flows JavaScript SDK is framework-agnostic. You call `init()` with your organization ID and user identity, and Flows handles everything else from there.
...
Call `init()` with your organization ID and user identity, then set up the component renderer and add the `<flows-floating-blocks>` element to your HTML:
...
For the full list of supported options, see the [JavaScript installation guide](/docs/sdk/javascript/installation). Once that is set up, come back here.
...
Now, when you start your app, you should see the checklist widget in the corner of the screen (it is possible that you will need to reload the page if you already had the app running before changing the environments). If you have any issues, you can debug by looking into the browser console or by using our SDK's [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your checklist looks and feels, you can publish it to your users. To do that, you first need to deploy the updated app with the `init()` integration (don't forget to change the environment option back to `production`).
...
That's it, you've successfully built a floating onboarding checklist in JavaScript using Flows.
...
If you're ready to ship better onboarding, faster, [try Flows](/technology/javascript-user-onboarding).

## Looking for other frameworks?

If you're building an onboarding checklist with a specific framework, check out our other guides:

- [How to build an onboarding checklist in React](/blog/react-onboarding-checklist-guide)
...
And if you want to guide users through a linear sequence of steps instead, see [How to build a product tour in JavaScript](/blog/javascript-product-tour-guide).

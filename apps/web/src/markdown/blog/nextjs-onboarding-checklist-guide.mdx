---
title: How to build an onboarding checklist in Next.js
description: A complete guide to building a user onboarding checklist in Next.js. Learn how to create a floating checklist widget, track task completion, and ship activation flows faster with Flows.
date: "2026-07-06"
author: "Ondrej Pesicka"
image: /images/blog/onboarding-checklist-guide/nextjs-cover.png
imageAlt: Floating onboarding checklist example in a Next.js application
---

In this guide, you'll learn how to build an onboarding checklist (also known as a floating checklist, setup guide, or getting started widget) in your Next.js application. Whether you're using the App Router or Pages Router, this tutorial will walk you through everything you need to know.
...
<Image
  ...
  alt="An example of a floating onboarding checklist in a Next.js application"
  width={1800}
  height={1040}
/>
...
Since Next.js apps are React apps, you can assemble the widget from components already in your design system (e.g. Popover, Checkbox, and Progress from `shadcn/ui`) and write custom logic for opening, collapsing, and persisting the widget across pages.

In Next.js, there's an extra consideration: the checklist depends on per-user state and browser APIs, so it must render as a client component after hydration. The best fit is usually a client component mounted in your root layout, so the widget survives route changes without remounting.
...
Create a client component that wraps your app with `FlowsProvider` and add it to your root layout:
...
For the full list of supported props and a complete working example, see the [Next.js installation guide](/docs/sdk/frameworks/next) or the [Next.js template project](https://github.com/RBND-studio/flows-sdk/tree/main/examples/react/next). When you have `<FlowsProvider>` integrated into your app, come back here.
...
Now, when you start your Next.js app, you should see the checklist widget in the corner of the screen (it is possible that you will need to reload the page if you already had the app running before changing the environments). If you have any issues, you can debug by looking into the browser console or by using our SDK's [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your checklist looks and feels, you can publish it to your users. To do that, you first need to deploy the updated Next.js app with `<FlowsProvider>` (don't forget to change the environment prop back to `production`).
...
That's it, you've successfully built a floating onboarding checklist in Next.js using Flows.
...
If you're ready to ship better onboarding, faster, [try Flows](/technology/nextjs-user-onboarding).
...
And if you want to guide users through a linear sequence of steps instead, see [How to build a product tour in Next.js](/blog/nextjs-product-tour-guide).

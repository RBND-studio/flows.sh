---
title: How to build an onboarding checklist in Angular
description: A complete guide to building a user onboarding checklist in Angular. Learn how to create a floating checklist widget, track task completion, and ship activation flows faster with Flows.
date: "2026-07-06"
author: "Ondrej Pesicka"
image: /images/blog/onboarding-checklist-guide/angular-cover.png
imageAlt: Floating onboarding checklist example in an Angular application
---

In this guide, you'll learn how to build an onboarding checklist (also known as a floating checklist, setup guide, or getting started widget) in your Angular application. Whether you're using Angular CLI (with or without `@angular/ssr`), NX, or a custom setup, this tutorial will walk you through everything you need to know.
...
<Image
  ...
  alt="An example of a floating onboarding checklist in an Angular application"
  width={1800}
  height={1040}
/>
...
Unlike tooltips or modals, there is no go-to Angular package for onboarding checklists. Most teams assemble one from components already in their design system (e.g. Menu, Checkbox, and Progress bar from `@angular/material`) and write custom logic for opening, collapsing, and persisting the widget across routes.
...
Create a `FlowsService` to handle initialization, call it from your root component, and add `<flows-floating-blocks>` to your root template:
...
For the full list of supported options, see the [Angular installation guide](/docs/sdk/frameworks/angular) or the [Angular template project](https://github.com/RBND-studio/flows-sdk/tree/main/examples/angular). Once that is set up, return here to continue.
...
Now, when you start your Angular app, you should see the checklist widget in the corner of the screen (it is possible that you will need to reload the page if you already had the app running before changing the environments). If you have any issues, you can debug by looking into the browser console or by using our SDK's [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your checklist looks and feels, you can publish it to your users. To do that, you first need to deploy the updated Angular app with the `init()` integration (don't forget to change the environment option back to `production`).
...
That's it, you've successfully built a floating onboarding checklist in Angular using Flows.
...
If you're ready to ship better onboarding, faster, [try Flows](/technology/angular-user-onboarding).
...
And if you want to guide users through a linear sequence of steps instead, see [How to build a product tour in Angular](/blog/angular-product-tour-guide).

---
title: How to build an onboarding checklist in Vue (Nuxt & Vite Guide)
description: A complete guide to building a user onboarding checklist in Vue (Nuxt or Vite). Learn how to create a floating checklist widget, track task completion, and ship activation flows faster with Flows.
date: "2026-07-06"
author: "Ondrej Pesicka"
image: /images/blog/onboarding-checklist-guide/vue-cover.png
imageAlt: Floating onboarding checklist example in a Vue application
---

In this guide, you'll learn how to build an onboarding checklist (also known as a floating checklist, setup guide, or getting started widget) in your Vue application. Whether you're using Nuxt, Vite, or another Vue setup, this tutorial will walk you through everything you need to know.
...
<Image
  ...
  alt="An example of a floating onboarding checklist in a Vue application"
  width={1800}
  height={1040}
/>
...
Unlike tooltips or modals, there is no go-to Vue package for onboarding checklists. Most teams assemble one from components already in their design system (e.g. Popover, Checkbox, and ProgressBar from `primevue`) and write custom logic for opening, collapsing, and persisting the widget across pages.

If you're using Nuxt, there's an extra consideration: the checklist depends on per-user state and browser APIs, so it should render client-side only, after the page has mounted. Otherwise you risk hydration mismatches between the server-rendered HTML and the client state.
...
Create a Nuxt plugin to initialize Flows on mount, add `<flows-floating-blocks>` to your root template, and register the custom elements in `nuxt.config.ts`:
...
For the full list of supported options, see the [Vue installation guide](/docs/sdk/frameworks/nuxt) or the [Vue template project](https://github.com/RBND-studio/flows-sdk/tree/main/examples/vue/nuxt). Once that is set up, return here to continue.
...
Now, when you start your Vue app, you should see the checklist widget in the corner of the screen (it is possible that you will need to reload the page if you already had the app running before changing the environments). If you have any issues, you can debug by looking into the browser console or by using our SDK's [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your checklist looks and feels, you can publish it to your users. To do that, you first need to deploy the updated Vue app with the `init()` integration (don't forget to change the environment option back to `production`).
...
That's it, you've successfully built a floating onboarding checklist in Vue using Flows.
...
If you're ready to ship better onboarding, faster, [try Flows](/technology/vue-user-onboarding).
...
And if you want to guide users through a linear sequence of steps instead, see [How to build a product tour in Vue](/blog/vue-product-tour-guide).

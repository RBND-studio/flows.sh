---
title: How to build a product tour in Svelte (SvelteKit & Vite Guide)
description: A complete guide to building a product tour in Svelte. Learn how to create tooltips, modals, onboarding flows, track user progress, and ship tours faster using Flows.
date: "2025-12-29"
author: "Vojtech Vidra"
image: /images/blog/product-tour-guide/svelte-cover.png
imageAlt: Tooltip example for a product tour in a Svelte application
---

import { VideoPlayer } from "components/blog";

In this guide, you’ll learn how to build a product tour (also known as user onboarding flow or a product adoption guide) in your Svelte application. Whether you’re using SvelteKit, Vite, or another Svelte setup, this tutorial will walk you through everything you need to know.
...
In Svelte, you have several options to create the UI for product tours. You can use dedicated packages like `flowbite-svelte`, `shepherd.js`, `intro.js`, or `driver.js`, or use the components already in your design system (e.g. Dialog or Popover from `shadcn-svelte`).
...
#### Product tours in Svelte vs SvelteKit

If you’re using SvelteKit, product tours require a bit of extra care around client-side rendering. Tour steps that depend on DOM elements should only run in the browser, after the page has mounted.

With Flows, this is handled automatically, tours wait for the relevant elements to exist before rendering, whether you’re using Svelte with Vite or SvelteKit with SSR enabled.
...
You might be tempted to store user state in the browser’s local storage, but for any Svelte app where users sign in (especially if they may use multiple devices) the state should be stored in your database instead.
...
Plus, for UI elements, you don't need to use our built-in components (even though we think they are really good). You can bring your own Svelte components directly from your design system. This gives you complete control to implement tours that meet your UI and UX requirements.
...
For the full list of supported options, see the [Svelte installation guide](/docs/sdk/frameworks/svelte) or the [Svelte template project](https://github.com/RBND-studio/flows-sdk/tree/main/examples/svelte). When you have `init()` integrated into your app, come back here.
...
After adding all your steps, the tour is ready for a preview. To preview the tour, create a testing environment called `stage` by going to **Settings > Environments**. Then change the `environment` prop in `init()` to match the newly created environment.
...
Now, when you start your Svelte app, you should see the first step of the tour (it is possible that you will need to reload the page if you already had the app running before changing the environments). If you have any issues, you can debug by looking into the browser console or by using our SDK’s [debug mode](/docs/sdk/debug).

### Step 4: Publish to production

Once you are happy with how your product tour looks and feels, you can publish it to your users. To do that, you first need to deploy the updated Svelte app with `init()` (don't forget to change the environment prop back to `production`).
...
<VideoPlayer
  ...
  ariaLabel="Preview of the product tour in a Svelte application"
/>

---

That’s it, you’ve successfully built a complete tour in Svelte using Flows.
